...
public class CombinedData extends BarLineScatterCandleBubbleData<IBarLineScatterCandleBubbleDataSet<? extends Entry>> {
    ...
    public void calcMinMax() {
        ...
        axisMax[0] = -Float.MAX_VALUE;
        axisMin[0] = Float.MAX_VALUE;
        axisMax[1] = -Float.MAX_VALUE;
        axisMin[1] = Float.MAX_VALUE;
        ...
        for (ChartData data : allData) {
            ...
            for (IBarLineScatterCandleBubbleDataSet<? extends Entry> dataset : sets) {
                if (dataset.getAxisDependency() == YAxis.AxisDependency.LEFT)  {
                    if (dataset.getYMax() > axisMax[0]) {
                        axisMax[0] = dataset.getYMax();
                    }

                    if (dataset.getYMin() < axisMin[0]) {
                        axisMin[0] = dataset.getYMin();
                    }
                }
                else {
                    if (dataset.getYMax() > axisMax[1]) {
                        axisMax[1] = dataset.getYMax();
                    }

                    if (dataset.getYMin() < axisMin[1]) {
                        axisMin[1] = dataset.getYMin();
                    }
                }
            }
        }
