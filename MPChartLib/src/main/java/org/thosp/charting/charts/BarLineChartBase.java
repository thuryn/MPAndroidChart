...
import org.thosp.charting.highlight.ChartHighlighter;
import org.thosp.charting.highlight.Highlight;
import org.thosp.charting.interfaces.dataprovider.BarLineScatterCandleBubbleDataProvider;
import org.thosp.charting.interfaces.datasets.IBarLineScatterCandleBubbleDataSet;
import org.thosp.charting.jobs.AnimatedMoveViewJob;
import org.thosp.charting.jobs.AnimatedZoomJob;
import org.thosp.charting.jobs.MoveViewJob;
import org.thosp.charting.jobs.ZoomJob;
import org.thosp.charting.listener.BarLineChartTouchListener;
import org.thosp.charting.listener.OnDrawListener;
import org.thosp.charting.renderer.XAxisRenderer;
import org.thosp.charting.renderer.YAxisRenderer;
import org.thosp.charting.utils.MPPointD;
...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
     ...
     */
    protected YAxis[] yAxes;

    protected YAxisRenderer[] axisRenderers;

    protected Transformer[] axisTransformers;
    ...
    public BarLineChartBase(Context context, int axisCount) {
        super(context);
    ...
    public void init() {
        ...
        yAxes = new YAxis[axisCount];
        axisTransformers = new Transformer[axisCount];
        axisRenderers = new YAxisRenderer[axisCount];
        for (int axisCounter = 0; axisCounter < axisCount; axisCounter++) {
            yAxes[axisCounter] = new YAxis(axisCounter);
            axisTransformers[axisCounter] = new Transformer(mViewPortHandler);
            axisRenderers[axisCounter] = new YAxisRenderer(mViewPortHandler, yAxes[axisCounter], axisTransformers[axisCounter]);
        }

        mXAxisRenderer = new XAxisRenderer(mViewPortHandler, mXAxis, axisTransformers[0]);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int axisCounter = 0; axisCounter < yAxes.length; axisCounter++) {
            if (yAxes[axisCounter].isEnabled())
                axisRenderers[axisCounter].computeAxis(yAxes[axisCounter].mAxisMinimum, yAxes[axisCounter].mAxisMaximum, yAxes[axisCounter].isInverted());
        }
        if (mXAxis.isEnabled())
            mXAxisRenderer.computeAxis(mXAxis.mAxisMinimum, mXAxis.mAxisMaximum, false);

        mXAxisRenderer.renderAxisLine(canvas);
        axisRenderers[0].renderAxisLine(canvas);
        axisRenderers[1].renderAxisLine(canvas);

        if (mXAxis.isDrawGridLinesBehindDataEnabled())
            mXAxisRenderer.renderGridLines(canvas);

        if (yAxes[0].isDrawGridLinesBehindDataEnabled())
            axisRenderers[0].renderGridLines(canvas);

        if (yAxes[1].isDrawGridLinesBehindDataEnabled())
            axisRenderers[1].renderGridLines(canvas);

        if (mXAxis.isEnabled() && mXAxis.isDrawLimitLinesBehindDataEnabled())
            mXAxisRenderer.renderLimitLines(canvas);

        if (yAxes[0].isEnabled() && yAxes[0].isDrawLimitLinesBehindDataEnabled())
            axisRenderers[0].renderLimitLines(canvas);

        if (yAxes[1].isEnabled() && yAxes[1].isDrawLimitLinesBehindDataEnabled())
            axisRenderers[1].renderLimitLines(canvas);
        ...
        axisRenderers[0].renderAxisLabels(canvas);
        axisRenderers[1].renderAxisLabels(canvas);
    ...
    protected void prepareValuePxMatrix() {
        ...
        for (int axisCounter = 0; axisCounter < axisTransformers.length; axisCounter++) {
            axisTransformers[axisCounter].prepareMatrixValuePx(mXAxis.mAxisMinimum,
                    mXAxis.mAxisRange,
                    yAxes[axisCounter].mAxisRange,
                    yAxes[axisCounter].mAxisMinimum);
        }
    }
    ...
    protected void prepareOffsetMatrix() {
        ...
        for (int axisCounter = 0; axisCounter < axisTransformers.length; axisCounter++) {
            axisTransformers[axisCounter].prepareMatrixOffset(yAxes[axisCounter].isInverted());
        }
    }
    ...
    public void notifyDataSetChanged() {
        ...
        for (int axisCounter = 0; axisCounter < axisRenderers.length; axisCounter++) {
            axisRenderers[axisCounter].computeAxis(yAxes[axisCounter].mAxisMinimum, yAxes[axisCounter].mAxisMaximum, yAxes[axisCounter].isInverted());
        }
        mXAxisRenderer.computeAxis(mXAxis.mAxisMinimum, mXAxis.mAxisMaximum, false);
    ...
    protected void autoScale() {
        ...
        for (int axisCounter = 0; axisCounter < yAxes.length; axisCounter++) {
            if (yAxes[axisCounter].isEnabled())
                yAxes[axisCounter].calculate(mData.getYMin(axisCounter),
                        mData.getYMax(axisCounter));
        }
    ...
    public void calculateOffsets() {
        ...
        if (!mCustomViewPortEnabled) {
            ...
            if (yAxes[0].needsOffset()) {
                offsetLeft += yAxes[0].getRequiredWidthSpace(axisRenderers[0]
                        .getPaintAxisLabels());
            }

            if (yAxes[1].needsOffset()) {
                offsetRight += yAxes[1].getRequiredWidthSpace(axisRenderers[1]
                        .getPaintAxisLabels());
            }
    ...
    public Transformer getTransformer(AxisDependency which) {
        if (which == AxisDependency.LEFT)
            return axisTransformers[0];
        else
            return axisTransformers[1];
    }

    public Transformer getTransformer(int which) {
        return axisTransformers[which];
    }
     ...
     */
    public void zoom(float scaleX, float scaleY, float xValue, float yValue, int axis) {
     ...
     */
    @TargetApi(11)
    public void zoomAndCenterAnimated(float scaleX, float scaleY, float xValue, float yValue, int axis,
                                      long duration) {
    ...
    public void setVisibleYRangeMaximum(float maxYRange, AxisDependency axis) {
        if (AxisDependency.LEFT == axis) {
            float yScale = getAxisRange(0) / maxYRange;
            mViewPortHandler.setMinimumScaleY(yScale);
        } else {
            float yScale = getAxisRange(1) / maxYRange;
            mViewPortHandler.setMinimumScaleY(yScale);
        }
    }
    ...
    public void setVisibleYRangeMinimum(float minYRange, AxisDependency axis) {
        if (AxisDependency.LEFT == axis) {
            float yScale = getAxisRange(0) / minYRange;
            mViewPortHandler.setMaximumScaleY(yScale);
        } else {
            float yScale = getAxisRange(1) / minYRange;
            mViewPortHandler.setMaximumScaleY(yScale);
        }
    }
    ...
    public void setVisibleYRange(float minYRange, float maxYRange, AxisDependency axis) {
        if (AxisDependency.LEFT == axis) {
            float minScale = getAxisRange(0) / minYRange;
            float maxScale = getAxisRange(0) / maxYRange;
            mViewPortHandler.setMinMaxScaleY(minScale, maxScale);
        } else {
            float minScale = getAxisRange(1) / minYRange;
            float maxScale = getAxisRange(1) / maxYRange;
            mViewPortHandler.setMinMaxScaleY(minScale, maxScale);
        }
    }
    ...
    public void moveViewTo(float xValue, float yValue, AxisDependency axis) {
        ...
        float yInView = getAxisRange(axisIndex) / mViewPortHandler.getScaleY();
     ...
     */
    @TargetApi(11)
    public void moveViewToAnimated(float xValue, float yValue, int axis, long duration) {
     ...
     * @param axis   - which axis should be used as a reference for the y-axis
     */
    public void centerViewToY(float yValue, int axis) {
     ...
     * @param axis   - which axis should be used as a reference for the y axis
     */
    public void centerViewTo(float xValue, float yValue, int axis) {
    ...
    public void centerViewToAnimated(float xValue, float yValue, AxisDependency axis, long duration) {
        ...
        MPPointD bounds = getValuesByTouchPoint(mViewPortHandler.contentLeft(), mViewPortHandler.contentTop(), axisIndex);
     ...
     * @return
     */
    protected float getAxisRange(int axis) {
        return yAxes[axis].mAxisRange;
    }
     ...
     * @return
     */
    public MPPointD getValuesByTouchPoint(float x, float y, int axis) {
        MPPointD result = MPPointD.getInstance(0, 0);
        getValuesByTouchPoint(x, y, axis, result);
        return result;
    }

    public void getValuesByTouchPoint(float x, float y, int axis, MPPointD outputPoint) {
     ...
     * @return
     */
    public MPPointD getPixelForValues(float x, float y, int axis) {
    ...
    public float getHighestVisibleX() {
        getTransformer(0).getValuesByTouchPoint(mViewPortHandler.contentRight(),
    ...
    /**
     * Get axis for left [0], right [1], third [2], etc.
     *
     * @param order
     * @return
     */
    public YAxis getAxisY(int order) {
        if (yAxes.length <= order) {
            YAxis[] newYAxes = new YAxis[order + 1];
            Transformer[] newAxisTransformers = new Transformer[order + 1];
            int axisCounter = 0;
            for (YAxis yAxis: yAxes) {
                if (axisCounter < order) {
                    newYAxes[axisCounter] = yAxes[axisCounter];
                    newAxisTransformers[axisCounter] = axisTransformers[axisCounter];
                } else {
                    newYAxes[axisCounter] = new YAxis(axisCounter);
                    newAxisTransformers[axisCounter] = new Transformer(mViewPortHandler);
                }
                axisCounter++;
            }
            yAxes = newYAxes;
            axisTransformers = newAxisTransformers;
        }
        return yAxes[order];
    }
    ...
    public boolean isInverted(AxisDependency axis) {
        if (axis == AxisDependency.LEFT) {
            return getAxis(0).isInverted();
        } else {
            return getAxis(1).isInverted();
        }
    }
    ...
    public void setRendererLeftYAxis(YAxisRenderer rendererLeftYAxis) {
        axisRenderers[0] = rendererLeftYAxis;
    }

    public YAxisRenderer getRendererRightYAxis() {
        return axisRenderers[1];
    }
    ...
    public void setRendererRightYAxis(YAxisRenderer rendererRightYAxis) {
        axisRenderers[1] = rendererRightYAxis;
    }

    @Override
    public float getYChartMax() {
        return Math.max(yAxes[0].mAxisMaximum, yAxes[1].mAxisMaximum);
    }

    @Override
    public float getYChartMin() {
        return Math.min(yAxes[0].mAxisMinimum, yAxes[1].mAxisMinimum);
    }
    ...
    public boolean isAnyAxisInverted() {
        if (yAxes[0].isInverted())
            return true;
        if (yAxes[1].isInverted())
            return true;
        return false;
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mKeepPositionOnRotation) {
            ...
            getTransformer(0).pixelsToValue(mOnSizeChangedBuffer);
        }
        ...
        if (mKeepPositionOnRotation) {
            ...
            getTransformer(0).pointValuesToPixel(mOnSizeChangedBuffer);
